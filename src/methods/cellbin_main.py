...
def s_main():
    ...
    parser.add_argument("-t", "--stain", required=True, choices=['he', 'ssdna', 'dapi', 'scell'], help="stain type")
    ...
    if len(img_list) > 1:
        iterator = tqdm(img_list, desc="Processing Images")
    else:
        iterator = img_list

    for img_path in iterator:
        name = os.path.splitext(os.path.basename(img_path))[0]
        c_mask_path = os.path.join(output_dir, f"{name}_v3_mask.tif")
        f_mask_path = os.path.join(output_dir, f"{name}_v3_corr_mask.tif")
        ...
        try:
            ...
            print(f"[Error] Failed processing: {img_path}, reason: {e}")
