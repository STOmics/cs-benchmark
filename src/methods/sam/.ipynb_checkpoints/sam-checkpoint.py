...
def sam_method(para, args):
    ...
    sam_checkpoint = os.path.join(os.path.abspath('.'), 'weights/sam/sam_vit_b_01ec64.pth')
    model_type = "vit_b"
    device = (para.is_gpu == True) and "gpu" or "cpu"
    device = 'gpu'
    ...
    for i in tqdm.tqdm(imgs, 'SAM'):
        ...
        for i, m in enumerate(masks): 
            mask[np.where(m['segmentation'] == True)] = i + 1
