...
Two command modes are `tile` and `wsi` to enter corresponding inference mode:
...
# 定义原有 tile 和 wsi 的参数说明字符串
tile_cli = """
Arguments for processing tiles.

usage:
    tile (--input_dir=<path>) (--output_dir=<path>) [--draw_dot] [--save_qupath] [--save_raw_map] [--mem_usage=<n>]
    
options:
   --input_dir=<path>     Path to input data directory. Assumes the files are not nested within directory.
   --output_dir=<path>    Path to output directory.
   --mem_usage=<n>        Declare how much memory (physical + swap) should be used for caching. [default: 0.2]
...
options:
    ...
    --mask_dir=<path>       Path to directory containing tissue masks. Should have the same name as corresponding WSIs. [default: '']
...
def convert_custom_args():
    """
    如果检测到用户传入的是自定义参数（如 -i, -o, -t, -g），
    则使用 argparse 解析后转换成 run_infer.py 所需的参数格式。
    """
    parser = argparse.ArgumentParser(description="Custom parameter conversion")
    ...
    parser.add_argument("-t", "--img_type", help="Image type, e.g., ss/he", required=True)
    ...
    # 将自定义参数映射到 HoVer-Net 需要的全局参数和子命令参数
    global_args = {
        '--gpu': custom_args.gpu,
    ...
    # 将全局参数转换为字符串列表，以便后续与子命令参数合并
    global_args_list = []
    for k, v in global_args.items():
        global_args_list.extend([k, v])
    
    # 针对子命令 tile 做不同的参数转换
    sub_args_list = [
        sub_cmd,
        '--input_dir', custom_args.input,
    ...
    sys.argv = new_argv
    # 可以打印 new_argv 检查转换效果
    # print("转换后的参数：", sys.argv)

def main():
    # 如果检测到命令行参数中含有自定义参数标识，则进行转换
    if any(arg in sys.argv for arg in ['-i', '--input']):
        convert_custom_args()

    # 以下部分不变，继续原有的逻辑
    sub_cli_dict = {'tile': tile_cli, 'wsi': wsi_cli}
    args = docopt(DOC, help=False, options_first=True, version='HoVer-Net Pytorch Inference v1.0')
    ...
    if sub_cmd == 'tile':
        ...
        run_args.update({
            'input_dir': sub_args['input_dir'],
            'output_dir': sub_args['output_dir'],
            'input_mask_dir': sub_args.get('input_mask_dir', ''),
